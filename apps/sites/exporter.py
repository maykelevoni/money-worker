...
def _optin_script(api_base, key):
    ...
    return (
        ...
        "var email=g('email'),lm=g('lead_magnet'),li=g('list_id'),sm=g('success_message')||\"You're in!\";"
        "var btn=f.querySelector('button');if(btn)btn.disabled=true;"
        "fetch(API+'/api/optin/',{method:'POST',"
        "headers:{'Content-Type':'application/x-www-form-urlencoded'},"
        "body:'site_key='+encodeURIComponent(KEY)+'&email='+encodeURIComponent(email)+'&lead_magnet='+encodeURIComponent(lm)+'&list_id='+encodeURIComponent(li)})"
